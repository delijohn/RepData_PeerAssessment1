...
For Total Steps per Day Density the mean is `r formatC(meanSteps,"%d")` and the median is `r medianSteps`
...
plot(avgi,type = "l")
```
The Interval that contains the maximum number of steps (on average) is: `r  maxInterval[1,1]`
...
The number of rows with NA's are: `r tb[1]`
